...
class MusicBotAgent(Agent):
    def __init__(self, agent_id: str = "music-bot"):
        ...
        available_commands = {
            'view_playlist' : playlist.Playlist(name='My Playlist', db_connection=db_connection).view_playlist()
        }

        tools = [playlist.Playlist(name='My Playlist', db_connection=db_connection).view_playlist()]

        prompt = f""" You are a music chatbot that will only answer questions about songs, albums or artists that are in the playlist or about songs that are in the database """

        mistral_model = ChatOllama('http://localhost:5000', model='mistral', num_ctx=4, temperature=0.2, system = prompt).bind_tools(tools=tools)
